...
class KineticModelFitterLM():
    def __init__(self, shape=(64, 64, 64), Nt=24):
        ...
        # number of parameters of the kinetic model
        self.D = 5
        self.shape = shape                                     # (image) shape
        # shape of observation data
        self.shape_y = (shape[0], shape[1], shape[2], self.Nt)
        # shape of 4D parameters matrix
        self.shape_par = (shape[0], shape[1], shape[2], self.D)
        self.eps = np.float32(1e-16)
        # weight lambda (LM algorithm)
        self.ll = np.float32(0.0)
        # weight beta (MRF prior importance)
        self.bb = np.float32(0.0)
    	# weight gamma (Thikonov term in MRF prior)
    	self.gamma = np.float32(0.0)
    	self.func_path = os.path.dirname(os.path.abspath(__file__))
    	self.kernel_models = open(
    	    self.func_path + '/cuda_kernels/models.cu').read()
    	self.kernel_priors = open(
    	    self.func_path + '/cuda_kernels/priors.cu').read()
        # allocate GPU memory
        self._init_gpu_()
    ...
    def _init_gpu_(self):
        ...
        self.y_gpu = gpuarray.empty(
            (self.N, self.Nt), np.float32)         # observations
        self.f_gpu = gpuarray.zeros(
            (self.N, self.Nt), np.float32)         # model evaluation
        self.mask_gpu = gpuarray.zeros(self.shape, np.float32)

        self.par_gpu = gpuarray.empty(
            (self.N, self.D, 1), np.float32)       # parameters
        self.par_out = np.zeros((self.N, self.D))
        self.dk_gpu = gpuarray.empty(1, np.float32)

        self.g_gpu = gpuarray.empty(
            (self.N, self.D, 1), np.float32)        # gradient
        self.g_arr = bptrs(self.g_gpu)
    	# MRF prior (same shape as self.g_gpu)
    	self.prior_gpu = gpuarray.empty((self.N, self.D, 1), np.float32)
        self.delta_gpu = gpuarray.empty(
            (self.N, self.D, 1), np.float32)        # gradient
        self.delta_arr = bptrs(self.delta_gpu)

    	self.Diff_gpu = gpuarray.empty(
    	    (self.N, self.Nt, 1), np.float32)       # self.y_gpu - self.f_gpu
        self.Diff_arr = bptrs(self.Diff_gpu)
        self.J_gpu = gpuarray.empty(
            (self.N, self.Nt, self.D), np.float32)  # Jacobian
        self.J_arr = bptrs(self.J_gpu)
        self.H_gpu = gpuarray.empty(
            (self.N, self.D, self.D), np.float32)   # (approximate) Hessian
        self.H_arr = bptrs(self.H_gpu)
        # (approximate) inverse Hessian
        self.Hinv_gpu = gpuarray.empty((self.N, self.D, self.D), np.float32)
        ...
        # info matrix utilized by batched cublas
        self._info_gpu = gpuarray.zeros(self.N, np.int32)
        # info matrix utilized by batched cublas
        self._p_gpu = gpuarray.empty((self.N, self.D), np.int32)
    ...
    def step(self, verbose=True, smoothing=0.0):
        ...
        self.parallel_analytic_models(block_dim=(
            self.B, 1, 1), grid_dim=(grid_dim, 1, 1))
        ...
        cublas.cublasSgetrfBatched(self._h, self.D, self.H_arr.gpudata,
                                   self.D, self._p_gpu.gpudata, self._info_gpu.gpudata, self.N)
        cublas.cublasSgetriBatched(self._h, self.D, self.H_arr.gpudata, self.D,
                                   self._p_gpu.gpudata, self.Hinv_arr.gpudata, self.D, self._info_gpu.gpudata, self.N)
    	...
    	self.compute_prior(block_dim=(self.B, 1, 1,), grid_dim=(
    	    np.int(np.ceil((self.N + self.B - 1) / self.B)), 1, 1))
    ...
    def parallel_analytic_models(self, block_dim=None, grid_dim=None):
        ...
        if block_dim is None or grid_dim is None:
            block_dim, grid_dim = cumisc.select_block_grid_sizes(
                pycuda.autoinit.device, self.shape)
	...
	if (self.model == "bicompartment_3expIF_4k") or (self.model == "bicompartment_2expIF_4k"):
	  ...
	  p1 = (K1 * (p2 - k3 - k4)) / d  # B1
	  p3 = (K1 * (-p4 + k3 + k4)) / d  # B2
	  self.par_gpu[:, :, 0] = gpuarray.to_gpu(np.asarray(
	      [[vB, p1, p2, p3, p4]] * self.N, dtype=np.float32))
    ...
    def aux2k(self):
	if (self.model == "bicompartment_3expIF_4k") or (self.model == "bicompartment_2expIF_4k"):
	  n = self.par_gpu.get()[:, 1, 0] * self.par_gpu.get()[:, 2, 0] + \
	                       self.par_gpu.get()[:, 3, 0] * \
	                                        self.par_gpu.get()[:, 4, 0]
	  d = self.par_gpu.get()[:, 1, 0] + self.par_gpu.get()[:, 3, 0]
	  self.par_out[:, 0] = self.par_gpu.get()[:, 0, 0]
	  self.par_out[:, 1] = d
	  self.par_out[:, 2] = n / d
	  self.par_out[:, 3] = (self.par_gpu.get()[:, 1, 0] * self.par_gpu.get()[:, 3, 0]
	                        * (self.par_gpu.get()[:, 2, 0] - self.par_gpu.get()[:, 4, 0])**2) / (d * n)
	  self.par_out[:, 4] = (self.par_gpu.get()[:, 2, 0] * self.par_gpu.get()[:, 4, 0]
	                        * (self.par_gpu.get()[:, 1, 0] + self.par_gpu.get()[:, 3, 0])) / n
	elif (self.model == "bicompartment_3expIF_3k") or (self.model == "bicompartment_2expIF_3k"):
	  self.par_out[:, 0] = self.par_gpu.get()[:, 0, 0]
	  self.par_out[:, 1] = self.par_gpu.get()[:, 1, 0] + \
	                                        self.par_gpu.get()[:, 3, 0]
	  self.par_out[:, 2] = self.par_gpu.get()[:, 1, 0] * \
	                                        self.par_gpu.get()[
	                                                         :, 2, 0] / self.par_out[:, 1]
	  self.par_out[:, 3] = self.par_gpu.get()[:, 2, 0] * \
	                                        self.par_gpu.get()[
	                                                         :, 3, 0] / self.par_out[:, 1]
	  self.par_out[:, 4] = 0
    ...
    def initialize(self, scanTime, observations, IF, IFparams=[], IF_model=2, model="bicompartment_3expIF_4k", prior="gaussian_MRF_prior", vB=0.01, K1=0.1, k2=0.1, k3=0.01, k4=0.01, dk=np.log(2) / 109.8, l=1e0, b=[0.1e0, 0.1e0, 0.1e0, 0.1e0, 0.1e0], g=1e0, t=[1e0, 1e1, 1e1, 1e0, 0], mask=None):
    	...
    	if len(b) == 1:
    	  ...
    	  self.bb = gpuarray.to_gpu(np.asarray(
    	      [[0.1e0, 0.1e0, 0.1e0, 0.1e0, 0.1e0]] * self.D, dtype=np.float32))
    	  print "Wrong size for beta array (prior weight)"
    	self.gamma = float32(g)
    	if len(t) == 1:
    	  self.thresh = gpuarray.to_gpu(
    	      np.asarray([t] * self.D, dtype=np.float32))
    	elif len(t) == self.D:
    	  self.thresh = gpuarray.to_gpu(np.asarray(t, dtype=np.float32))
    	else:
    	  self.thresh = gpuarray.to_gpu(np.asarray(
    	      [[1e0, 1e1, 1e1, 1e0, 0]] * self.D, dtype=np.float32))
    	...
    	if mask is None:
    	  self.mask_gpu = gpuarray.to_gpu(np.asarray(
    ...
    def set_input_function(self, IF, scanTime, IFparams, IF_model=2):
    	...
    	if (IFparams == []) or (IFparams == None):
    	  ...
    	  self.IFparams_gpu = gpuarray.to_gpu(
